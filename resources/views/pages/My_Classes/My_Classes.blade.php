    ...
    <div class="card card-statistics h-100">
        <div class="card-body">
            ...
            <form action="{{ route('Filter_Classes') }}" method="POST">
                {{ csrf_field() }}
                <select class="selectpicker button x-small" data-style="btn-info" name="Grade_id" required
                    onchange="this.form.submit()">
                    <option value="" selected disabled>{{ trans('My_Classes_trans.Search_By_Grade') }}</option>
                    @foreach ($Grades as $Grade)
                        <option class=" button x-small" value="{{ $Grade->id }}">{{ $Grade->Name }}</option>
                    @endforeach
                </select>
            </form>
            ...
            <div class="table-responsive">
                <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                    ...
                    <thead>
                        <tr>
                            <th><input name="select_all" id="example-select-all" type="checkbox"
                                    onclick="CheckAll('box1', this)" /> {{ trans( 'My_Classes_trans.delete' ) }}   </th>
                                    {{-- to Choose when want to delete   --}}
                            <th>#</th>
                    ...
                    <tbody>
                        ...
                        @if (isset($details))
                            <?php $List_Classes = $details;     ?>      {{--  when choose all classesrooms  --}}
                        @else
                            <?php $List_Classes = $My_Classes;  ?>      {{--  but here when choosse search  --}}
                        @endif
                        <?php $i = 0; ?>                                {{--  create counter in table       --}}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class=" row mb-30" action="{{ route('Classroom.store') }}" method="POST">
                        ...
                        <div class="card-body">
                            <div class="repeater">
                                <div data-repeater-list="List_Classes">   {{-- add many rows return array have many rows --}}
    ...
    $(function() {
        $("#btn_delete_all").click(function() {
            ...
            if (selected.length > 0) {
                $('#delete_all').modal('show')                         // show Model
                $('input[id="delete_all_id"]').val(selected);         // get all id when chosse
            }
        });
    });
</script>
