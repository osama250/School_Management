...
class ReceiptStudentsRepository implements ReceiptStudentsRepositoryInterface
{
    ...
    public function store($request)
    {
        ...
        try
        {
            ...
            $receipt_students->Debit           = $request->Debit;   // مدييين
            $receipt_students->description     = $request->description;
            $receipt_students->save();

            // حفظ البيانات في جدول الصندوق
            $fund_accounts = new FundAccount();                     //  استلام المببلغ
            $fund_accounts->date               = date('Y-m-d');
            $fund_accounts->receipt_id         = $receipt_students->id;
            $fund_accounts->Debit              = $request->Debit;  //  بسجل المدين فى الحالة استلام المبلغ
            $fund_accounts->credit             = 0.00;             // والداءن بيكون 0
            ...
            $fund_accounts->date               = date('Y-m-d');
            $fund_accounts->type               = 'receipt';             // نوع الفاتورة
            $fund_accounts->receipt_id         = $receipt_students->id;
            $fund_accounts->student_id         = $request->student_id;
            $fund_accounts->Debit              = 0.00;                  //  المديين هنا ب 0
            $fund_accounts->credit             = $request->Debit;       //  داين بمبلغ  اللى هيدفعو
            $fund_accounts->description        = $request->description;
            $fund_accounts->save();
